...
describe("POST /sign-in", ()=>{
    ...
    beforeAll(async ()=>{
        await connection.query(`INSERT INTO usuario (name, email, password) VALUES ($1, $2, $3);`,[validBody.name, validBody.email, password1]);
    });
    ...
    test("returns 401 when there is no account email", async ()=>{
        const result = await supertest(app).post('/sign-in').send({email: "xx"+validBody.email, password: validBody.password});

        expect(result.status).toEqual(401);
    });  
    
    test("returns 401 when there wrong password", async ()=>{
        const result = await supertest(app).post('/sign-in').send({email: validBody.email, password: "catchau"});
